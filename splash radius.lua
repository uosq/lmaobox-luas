local radius = {
    [E_WeaponBaseID.TF_WEAPON_ROCKETLAUNCHER] = 146,
    [E_WeaponBaseID.TF_WEAPON_DIRECTHIT] = 44,
}

local team = {
    [E_TeamNumber.TEAM_RED] = {0, 150, 255}, --- blu team
    [E_TeamNumber.TEAM_BLU] = {255, 0, 0}, --- red team
    [E_TeamNumber.TEAM_SPECTATOR] = {255, 255, 255},
    [E_TeamNumber.TEAM_UNASSIGNED] = {255, 255, 255},
}
...
local minSegments = 3
local maxSegments = 50

local function draw_3d_circle(center_x, center_y, center_z, radius, normal_vector, distance)
   local num_segments = math.floor(maxSegments / (1 + distance * 0.01))
   num_segments = math.max(minSegments, math.min(maxSegments, num_segments))
...
local function Draw()
   ...
   for _, rocket in pairs(rockets) do
    local launcher = rocket:GetPropEntity("m_hLauncher")
    if launcher then
        local pos = rocket:GetAbsOrigin()
        local color = team[launcher:GetTeamNumber()]
        draw.Color(color[1], color[2], color[3], 255)
        draw_3d_circle(pos.x, pos.y, pos.z, radius[launcher:GetWeaponID()] or 146, {0, 0, 1}, (pos - localPos):Length())
      end
   end
end
