    ...
    do --- player weapons
        if entity:IsWeapon() then
            if entity:IsMeleeWeapon() then
                ...
                return entity:GetLoadoutSlot() == E_LoadoutSlot.LOADOUT_POSITION_PRIMARY and colors.PRIMARY_WEAPON or
                    colors.SECONDARY_WEAPON
            end
        end
    end
...
local mat = materials.Create("outlinetest", [["UnlitGeneric"
{
...
local flat = materials.Create("flatmat", [[UnlitGeneric
{
   $basetexture "vgui/white_additive"
}]])
...
local function ShouldDraw(entity)
   ...
   if attachments[entity:GetIndex()] == 0 then
    ...
    if entity:IsPlayer() then
        local team = entity:GetTeamNumber()
        if team == pteam and enemy_only then return false end
        ...
        if buildings and (class == "CObjectSentrygun" or class == "CObjectTeleporter" or class == "CObjectDispenser") and entity:GetHealth() > 0 then
            return true
        end
    end
...
callbacks.Register("DrawModel", function(dme)
    if dme:IsDrawingGlow() then return end
    ...
    local selectedcolor = dme:IsDrawingBackTrack() and colors.BACKTRACK
        or dme:IsDrawingAntiAim() and colors.ANTIAIM or getentitycolor(entity)
